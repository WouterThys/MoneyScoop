...
{
    public class DataList<T> : IDataList, IEnumerable<T> where T : ISaveableObject
    {
        ...
        public DataList()
        {
            Name = ObjectDefinitions.GetTableName<T>();
            NeedsFetch = true;
        }
        ...
        {
            DataList<T> copy = new DataList<T>();
            lock (locker)
            {
        ...
        public T ByIndex(int index)
        {
            lock (locker)
            {
                List<T> vals = new List<T>(dictionary.Values);
                return vals[index];
            }
        }
        ...
        public IEnumerable<T> ModifiedAfter(DateTime date)
        {
            lock (locker)
            {
                if (date == null) return dictionary.Values;
                return dictionary.Values.Where(t => t.LastModified > date);
            }
        }
